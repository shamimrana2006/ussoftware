...
export default function RegisterPage() {
  ...
  const registerT = t.registerPage;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!name || !email || !password) {
      setErrorMsg(isEn ? "Please fill in all required fields." : "প্রয়োজনীয় ঘরগুলো পূরণ করুন।");
      return;
    }

    if (!agreed) {
      setErrorMsg(isEn ? "Please agree to the Terms of Service to continue." : "অনুগ্রহ করে শর্তাবলীতে সম্মতি জানান।");
      return;
    }
    ...
    setTimeout(() => {
      setIsLoading(false);
      setSuccessMsg(registerT.registerSuccess);
    }, 1200);
  };
  ...
  return (
    <div className="min-h-screen bg-[#f0f7ff] flex flex-col font-sans select-none overflow-x-hidden relative">
      ...
      <main className="flex-grow flex items-center justify-center py-12 sm:py-20 px-4 relative z-10">
        ...
        {/* ========================================================================= */}
        {/* ETHEREAL SKY BACKGROUND WITH SOFT CLOUDS & GLOWING LIGHT ARCS             */}
        {/* ========================================================================= */}
        <div className="absolute inset-0 pointer-events-none -z-10 overflow-hidden">
          ...
          {/* Soft Ethereal Glowing Light Arc */}
          ...
          <motion.div
            ...
            className="absolute top-20 right-10 w-96 h-48 bg-white/40 blur-3xl rounded-full pointer-events-none"
          />
          <motion.div
            animate={{ x: [0, -30, 0], y: [0, 15, 0] }}
            transition={{ repeat: Infinity, duration: 20, ease: "easeInOut" }}
            className="absolute bottom-20 left-10 w-[450px] h-60 bg-white/50 blur-3xl rounded-full pointer-events-none"
          />
        </div>

        {/* ========================================================================= */}
        {/* CENTERED FROSTED GLASS CARD FOR REGISTRATION                              */}
        {/* ========================================================================= */}
        <motion.div
          initial={{ opacity: 0, y: 24, scale: 0.96 }}
          animate={{ opacity: 1, y: 0, scale: 1 }}
          transition={{ duration: 0.6, ease: "easeOut" }}
          className="w-full max-w-lg bg-white/80 backdrop-blur-2xl border border-white/90 shadow-[0_25px_70px_rgba(0,0,0,0.07)] rounded-[32px] p-7 sm:p-10 relative overflow-hidden my-4"
        >
          {/* Mode Switcher Pill */}
          ...
          <div className="flex justify-center mb-4">
            <div className="w-13 h-13 sm:w-14 sm:h-14 bg-gradient-to-tr from-[#008744] to-[#056839] text-white rounded-2xl flex items-center justify-center shadow-md border border-emerald-400/40 group">
              <UserPlus size={22} className="text-white group-hover:scale-110 transition-transform" />
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1 text-left">
              <label className="text-xs font-bold text-slate-700 ml-1">
                {registerT.nameLabel} <span className="text-rose-500">*</span>
              </label>
              <div className="relative flex items-center bg-slate-100/90 hover:bg-slate-100 focus-within:bg-white border border-slate-200/90 focus-within:border-[#008744] focus-within:ring-2 focus-within:ring-[#008744]/20 rounded-xl px-3.5 py-3 transition-all">
                <User size={18} className="text-slate-400 mr-2.5 flex-shrink-0" />
                <input
                  type="text"
                  value={name}
                  onChange={(e) => setName(e.target.value)}
                  placeholder={registerT.namePlaceholder}
            ...
            <div className="space-y-1 text-left">
              <label className="text-xs font-bold text-slate-700 ml-1">
                {registerT.emailLabel} <span className="text-rose-500">*</span>
              </label>
              <div className="relative flex items-center bg-slate-100/90 hover:bg-slate-100 focus-within:bg-white border border-slate-200/90 focus-within:border-[#008744] focus-within:ring-2 focus-within:ring-[#008744]/20 rounded-xl px-3.5 py-3 transition-all">
            ...
            <div className="space-y-1 text-left">
              <label className="text-xs font-bold text-slate-700 ml-1">
                {registerT.phoneLabel}
              </label>
              <div className="relative flex items-center bg-slate-100/90 hover:bg-slate-100 focus-within:bg-white border border-slate-200/90 focus-within:border-[#008744] focus-within:ring-2 focus-within:ring-[#008744]/20 rounded-xl px-3.5 py-3 transition-all">
                <Phone size={18} className="text-slate-400 mr-2.5 flex-shrink-0" />
                <input
                  type="tel"
                  value={phone}
            ...
            <div className="space-y-1 text-left">
              <label className="text-xs font-bold text-slate-700 ml-1">
                {registerT.passwordLabel} <span className="text-rose-500">*</span>
              </label>
              <div className="relative flex items-center bg-slate-100/90 hover:bg-slate-100 focus-within:bg-white border border-slate-200/90 focus-within:border-[#008744] focus-within:ring-2 focus-within:ring-[#008744]/20 rounded-xl px-3.5 py-3 transition-all">
            ...
            <div className="flex items-start gap-2.5 pt-1 text-left">
              <input
                ...
                className="mt-0.5 w-4 h-4 rounded text-[#008744] focus:ring-[#008744] border-slate-300 cursor-pointer"
              />
              <label htmlFor="terms" className="text-xs font-medium text-slate-600 leading-tight cursor-pointer">
                {registerT.termsAgree}
              </label>
            </div>
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full bg-[#008744] hover:bg-[#007038] text-white py-3.5 rounded-xl font-extrabold text-sm shadow-md hover:shadow-lg transition-all duration-200 cursor-pointer flex items-center justify-center gap-2 mt-4"
            >
              {isLoading ? (
                <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
              ) : (
                <>
                  <span>{registerT.submitBtn}</span>
                  <ArrowRight size={16} />
                </>
              )}
            </button>
          ...
          <div className="text-center mt-6 text-xs font-medium text-slate-600">
            <span>{registerT.alreadyHaveAccount} </span>
            <Link
              href="/login"
